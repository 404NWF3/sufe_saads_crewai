"""CTINexus adapter: generate per-item knowledge graphs.

Self-contained package under backend/; no imports from the legacy crew package
or from intel_agent.
"""
...
class CtinexusKgGenerator:
    ...
    def generate_for_item(
        ...
        local_item = coerce_raw_item(item)
        decision = assess_kg_eligibility(local_item, self.config, target_topics=target_topics)
        record = self._base_record(run_id, local_item, decision)
        ...
        output_path = self._item_json_path(run_id, local_item.item_id)
        ...
        try:
            ...
            self._copy_graph_html(record, result, run_id, local_item.item_id)
    ...
    def _load_ctinexus(self) -> ProcessCtiReport:
        try:
            ...
            raise RuntimeError(
                ...
                "Install with `uv sync --group ctinexuskg` before enabling KG generation."
            ) from exc
