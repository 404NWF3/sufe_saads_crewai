...
Providers (INTEL_SDK_PROVIDER, auto-detected when unset):
...
All claude_agent_sdk imports are lazy so the package (and the rules-only
fallback) works without the `agentsdk` dependency group installed.
"""
...
def selected_provider() -> ProviderSpec:
    ...
    if explicit:
        if explicit not in PROVIDERS:
            raise ValueError(f"INTEL_SDK_PROVIDER={explicit!r} is not one of {sorted(PROVIDERS)}")
...
def main_model() -> str:
    ...
    return _setting("INTEL_SDK_MODEL") or _setting(spec.model_var) or spec.default_main_model


def fast_model() -> str:
    spec = selected_provider()
    return _setting("INTEL_SDK_FAST_MODEL") or _setting(spec.fast_model_var) or spec.default_fast_model
...
def sdk_runtime_available() -> tuple[bool, str]:
    """Cheap engine-level availability probe (no network call)."""
    if not api_key():
        spec = selected_provider()
