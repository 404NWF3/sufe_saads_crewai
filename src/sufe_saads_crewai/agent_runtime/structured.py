...
Replaces real_loop._kickoff_json as the decision engine while keeping the same
contract: any failure (runtime unavailable, timeout, parse error, validation
error) returns None so the caller falls back to the shared deterministic rules.
Unlike the CrewAI path, every failure is counted and exposed via telemetry.
"""
...
DEFAULT_DECISION_TIMEOUT_SECONDS = 240.0
...
def _run_coro_sync(coro: Any, timeout_seconds: float) -> Any:
    """Run a coroutine from sync code, also when an event loop is already running."""
...
async def _sdk_decision_call(
    ...
    """Tool-forcing call: the agent must submit its decision through one MCP tool."""
    ...
    if captured:
        return captured[0]
    # Dual mode: some models reply with raw JSON despite tool forcing.
    return "\n".join(final_text) or None


class StructuredDecisionEngine:
    """Issues structured decisions through claude-agent-sdk with full telemetry."""
    ...
    def __init__(
        ...
        # Test seam: async callable (prompt, schema_dict, system_prompt, model, max_turns) -> dict | str | None
...
def _tool_input_schema(output_model: type[BaseModel]) -> dict[str, Any]:
    ...
    # MCP tool input schemas do not support $defs references everywhere;
    # inline them so nested models (e.g. lists of proposals) stay valid.
